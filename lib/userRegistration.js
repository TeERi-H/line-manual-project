...
export class UserRegistrationHandler {
  ...
  validateEmail(email) {
    ...
    if (domain !== 'vix.co.jp') {
      return {
        valid: false,
        error: `@vix.co.jpドメインのメールアドレスのみ登録可能です。\n\n会社から支給されたメールアドレスを入力してください。`
      };
    }
...
/**
 * ユーザー登録ハンドラーのシングルトンインスタンス（遅延初期化）
 */
let userRegistrationHandlerInstance = null;
export const getUserRegistrationHandler = () => {
  if (!userRegistrationHandlerInstance) {
    userRegistrationHandlerInstance = new UserRegistrationHandler();
  }
  return userRegistrationHandlerInstance;
};