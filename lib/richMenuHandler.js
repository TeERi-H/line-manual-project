...
export class RichMenuHandler {
  ...
  async handlePopularManualsAction(replyToken, user) {
    ...
    try {
      ...
      // 人気マニュアル（アクセス頻度順）を取得
      const popularResult = await this.getPopularManuals(user);
      
      if (!popularResult.success || popularResult.manuals.length === 0) {
        await this.getClient().replyMessage(replyToken, {
          type: 'text',
  ...
  async getUserStats(userId) {
    try {
      ...
      // デモ用の統計情報
      const stats = {
        totalSearches: Math.floor(Math.random() * 100) + 1,
        topCategory: ['経理', '人事', 'IT', '総務', '営業'][Math.floor(Math.random() * 5)],
