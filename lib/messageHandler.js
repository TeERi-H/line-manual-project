...
import { richMenuHandler } from './richMenuHandler.js';
...
export class MessageHandler {
   ...
   */
  async handleStickerMessage(event) {
    console.log(`🔖 Sticker message from ${event.source.userId}`);
    
    // スタンプには簡単な応答
    const client = this.getClient();
    await client.replyMessage(event.replyToken, {
      type: 'text',
      text: 'スタンプありがとうございます！😊\n何かお手伝いできることがあればお聞かせください。'
    });
   ...
   */
  async handleImageMessage(event) {
    console.log(`🖼️ Image message from ${event.source.userId}`);
   ...
   */
  async handleAudioMessage(event) {
    console.log(`🔊 Audio message from ${event.source.userId}`);
    
    const client = this.getClient();
    await client.replyMessage(event.replyToken, {
      type: 'text',
      text: '音声メッセージをありがとうございます！\nテキストでのお問い合わせにお答えできます。'
    });
  ...
  async handleUnfollowEvent(event) {
    const userId = event.source.userId;
    console.log(`👋 Unfollow event from ${userId}`);
  ...
  async handlePostbackEvent(event) {
    ...
    console.log(`📲 Postback event from ${userId}, data: ${data}`);
